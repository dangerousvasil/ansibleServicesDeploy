Доброго дня сегодня мы будем говорить об ансибл и сервисах, можно конечно использовать и другие запчасти для соа типа докера, дженкинса и папета, но сегодня у нас ансибл, сервисы и пару строчек пхп.
...
где упоминается SOA. Хотел взять картинку из статьи, но наверное с wiki картинкой будет меньше проблем.

Так и ближе к делу сегодня мы будем 
## Деплоить SOA проект
2 nginx , 2 фронтов и 2 сервисов и 1 для деплоя.
Всего 7 вм понадобиться для разворачивания соа песочницы.

В качестве деплой сервера будем использовать локальную машину и в томже сегменте сети у нас развернуты виртуалки виртуалбокс с пользователем ansible#ansible и 2 сетевых интерфейса с NAT(интернет) и внутренней сетью 192.168.0.0\32.
Если переиспользовать ВМ то можно сократить это число до 4 в качестве песочницы, но не более.
Также нашим ВМ требуется доступ в интернет для устнавки софта самый просто способ это сделать 2 сетевых интерфейса с NAT и внутренней сетью.
...
Они содержат в себе пару строчек пхп кода без привязки к какому либо фреймворку или проекту
...
Задача сервиса ответить пару слов что он думает и точка.
...
```
Как видете я переиспользую ВМ машины для сервиса и для форонта, но давйте будем думать что это разные вм который находяться на разных железных серверах для отказо устойчивости.
...
для хостов входящих в группу php-cli установить консольный вариант пхп для крон заданий например. 
...
Если вы хотите сделать административную панель сервиса то ненадо делать для неё дополнительный сервис можно использовать этиже ВМ и обрашаться к ним как {{service_name}}_web 
...
```
специально для вас я уже сгенерировал ключи в папке roles/authorized_key/keys, но вы ~~обязаны~~можете сделать и свои.
...
```
Теперь наш фронт начал почсылать запросы на наш фронт app но там ещё ничего нету ....
...
```
Зайдя на машинку мы увидим что эта папка у нас являеться ссылкой которая видете на другую папку 
...
## Балансер
Код выложен фронт настроен осталось дело за малым балансировшик сервисов.
Пока используем 1 можно добавить ещё несколько и банасировать между ними с помошью разных инструментов ddns или ха прокси или просто рандомно выбирать сервер на стороне приложения решать вам.
Запускаем задачу 
...
## Окружение
для того чтобы наше приложение знало что её окружает балансеры сервисы мы сгенерируем небольшой ини файл и разложим его на каждую машинку 
```
...
для когото это кажеться полной ерундой, а ктото увидит что наш сервис написал нам свои 3 большие буквы "API" и обрадуется.


## Заключение
Если вы добавили свой проект в качестве фронта и написали там пару строчек или подключили пару бибилиотек для работы с сервисами по json-rpc, наверно вы разбираетесь в пхп и вам это сильно поможет
```
$ch = curl_init();
...
```
То поздравляю вы окунули свой проект в самый модный и молодежный тренд сезона СОА.

### Пост скриптум 
Код проекта ещё требует множества доработок и конфигураций, поэтому сделаны только основные конфигурации чтоб работало.